...
def norm_adj(adj):
    ...
    Accepts torch.Tensor of shape (..., N, N) or numpy array. Returns torch.Tensor
    on the same device if input is torch.Tensor.
    """
    import torch
    if isinstance(adj, np.ndarray):
        adj = torch.tensor(adj, dtype=torch.float32)
    if not isinstance(adj, torch.Tensor):
        raise ValueError('adj must be numpy array or torch.Tensor')
    ...
    inv_sqrt_deg = inv_sqrt_deg.unsqueeze(-1)
    if adj.dim() == 2:
        D_half = torch.diag(inv_sqrt_deg.squeeze(-1))
        return D_half @ adj @ D_half
    else:
        # batch matmul: (B,N,1) * (B,1,N) -> (B,N,N)
        D_left = inv_sqrt_deg * torch.eye(adj.size(-1), device=orig_device)
        # broadcast D_left to batch
        D_left = D_left.unsqueeze(0) if adj.dim() == 3 and D_left.dim() == 2 else D_left
        return (inv_sqrt_deg * adj) * inv_sqrt_deg.transpose(-1, -2)
