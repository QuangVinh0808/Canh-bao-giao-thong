...
def get_float_adj(distance_df, num_nodes, node_order_path=None):
    """Build a symmetric float adjacency matrix from a distance DataFrame.

    distance_df: pandas.DataFrame or array-like with rows [i, j, d]
    num_nodes: expected number of nodes
    node_order_path: optional path to file listing original node ids (one per line)
    """
    # build id->index mapping if node_order provided
    ...
    values = distance_df.values if hasattr(distance_df, 'values') else np.array(distance_df)
    for row in values:
        if len(row) < 3:
            continue
        i, j, d = row[0], row[1], row[2]
        try:
            ii = idx2nodeid.get(int(i), int(i))
            jj = idx2nodeid.get(int(j), int(j))
        except Exception:
            try:
                ii = int(i)
                jj = int(j)
        ...
        try:
            ...
            val = 0.0
        if 0 <= ii < num_nodes and 0 <= jj < num_nodes:
            adj[ii, jj] = val
            adj[jj, ii] = val
    return adj
...
def get_dataloader(train, val):
    # process signal
    if args.dataset in ('PEMS03', 'PEMS04', 'PEMS08', 'my_data'):
    ...
    if args.dataset in ('PEMS03', 'PEMS04', 'PEMS08', 'my_data'):
        ...
        # For hypergraph model, provide a list of predefined adjacency matrices
        args.predefined_adjs = [adj_with_self_loop, adj_with_self_loop]
    else:
        raise NotImplementedError
