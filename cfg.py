...
parser.add_argument('--dataset', type=str, default='my_data', choices=['PEMS03', 'PEMS04', 'PEMS08', 'my_data'], help='dataset')
...
if isinstance(args.device, int) and args.device >= 0 and torch.cuda.is_available():
...
if args.dataset == 'PEMS08':
    ...
    args.num_nodes = 3  # 3 Node
    args.in_dim = 3     # occupancy, speed, flow
    args.out_dim = 1    # flow
    args.start_time = datetime(2025, 12, 18, 15, 30, 0)  # set according to your data
    args.time_delta = timedelta(seconds=10)           # sampling period set to 10 seconds per user request
else:
